 ...
 *
 * 两种解法 一种直接根据关系暴力求解 另一种是类比纸币问题贪心求解。
 * */
...
static char * intToRoman(int num){
	int i = 1000, j = 0, n = 0;
	char *roman = NULL;
	roman = (char*)malloc(sizeof(*roman)*20);
	if (NULL == roman)
	      return NULL;
	...
	for (; n < 4; n++){
		if(num >= i) {
			...
			if (j > 5){
				if (j == 9) {
					strncat(roman, rom+2, 1);
					strncat(roman, rom, 1);
					goto out;
				} else {
					strncat(roman, rom+1, 1);
					j = j - 5;
				}
			} else if (j == 5) {
				strncat(roman, rom+1, 1);
...
int main(void){
	int num = 0;
	char *roman;
	printf("please input a number less than 3999!");
	scanf("%d", &num);
	if( num > 3999 || num < 1 ){
		printf("num is not proper. exit");
		return -1;
	}
	roman = intToRoman(num);
	printf("num %d to roman is %s  .\n", num, roman);
	return 0;
